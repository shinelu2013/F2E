...
export default {
  ...
  methods: {
    ...
    removeTag(tag) {
      ...
      switch (tag.kind) {
        case 'location':
          ...
          break;
        case 'categories':
          this.filterCategories = this.filterCategories.filter(item => item != tag.value);
          break;
        case 'text':
          this.filterText = '';
          break;
    ...
    filterData() {
      ...
      if (this.filterLocation) {
        ...
        tempDataSet = tempDataSet.filter(item => item.Zone === this.filterLocation);
      ...
      if (this.filterCategories.length > 0) {
        ...
        tempDataSet = tempDataSet.filter(item => this.filterCategories.indexOf(item.Class1) != -1);
        console.log('filterd length: ', tempDataSet.length);
      }
      if (this.filterText) {
        ...
        tempDataSet = tempDataSet.filter(item => item.Name.indexOf(this.filterText) != -1);
        console.log('filterd length: ', tempDataSet.length);
      }
  ...
  computed: {    
    pageData() {
      this.isChange = false;
      const start = (this.displayPage - 1) * this.itemPerPage;
